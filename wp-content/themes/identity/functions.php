...
add_action( 'admin_menu', 'identity_post_format_instagram_field' );
add_action( 'save_post', 'identity_save_post_format_instagram_field', 10, 2 );
...
function identity_post_format_instagram_field() {	
	add_meta_box( 'my-instagram-box', 'Instagram URL', 'identity_create_post_format_instagram_field', 'post', 'normal', 'high' );
	add_meta_box( 'my-instagram-box', 'Instagram URL', 'identity_create_post_format_instagram_field', 'portfolio', 'normal', 'high' );	
}
...
function identity_create_post_format_instagram_field( $object, $box ) { ?>
	<p>
		<label for="post-instagram">Instagram URL</label>
		<br />
		<input name="post-instagram" id="post-instagram" cols="60" rows="4" tabindex="30" style="width: 97%;" value="<?php echo esc_html( get_post_meta( $object->ID, 'Instagram URL', true ) ); ?>" />
		<input type="hidden" name="sentient_meta_box_instagram" value="<?php echo wp_create_nonce( plugin_basename( __FILE__ ) ); ?>" />
	</p>
<?php }
...
function identity_create_team_socialone( $object, $box ) { ?>
	<p>
		...
		<select name="teamsocialone-shortcode" id="teamsocialone-shortcode" cols="60" rows="4" tabindex="30" style="width: 97%;">
			...
			<option value="instagram" <?php if(get_post_meta( $object->ID, 'Team Social One', true ) == 'instagram'){ ?> selected="selected" <?php } ?>>Instagram</option>				
		</select>		
...
function identity_create_team_socialtwo( $object, $box ) { ?>
	<p>
		...
		<select name="teamsocialtwo-shortcode" id="teamsocialtwo-shortcode" cols="60" rows="4" tabindex="30" style="width: 97%;">
			...
			<option value="instagram" <?php if(get_post_meta( $object->ID, 'Team Social Two', true ) == 'instagram'){ ?> selected="selected" <?php } ?>>Instagram</option>				
		</select>		
...
function identity_create_team_socialthree( $object, $box ) { ?>
	<p>
		...
		<select name="teamsocialthree-shortcode" id="teamsocialthree-shortcode" cols="60" rows="4" tabindex="30" style="width: 97%;">
			...
			<option value="instagram" <?php if(get_post_meta( $object->ID, 'Team Social Three', true ) == 'instagram'){ ?> selected="selected" <?php } ?>>Instagram</option>				
		</select>		
...
function identity_save_post_format_instagram_field( $post_id, $post ) {

	if(isset ($_POST['sentient_meta_box_instagram'])){
		if ( !wp_verify_nonce( $_POST['sentient_meta_box_instagram'], plugin_basename( __FILE__ ) ) )
			return $post_id;
	}	
	...
	$meta_value = get_post_meta( $post_id, 'Instagram URL', true );
	
	if(isset ($_POST['post-instagram'])){
		$new_meta_value = stripslashes( $_POST['post-instagram'] );
	} else {
		$new_meta_value = '';
	}											

	update_post_meta( $post_id, 'Instagram URL', $new_meta_value );
...
function Identity_Generate_CSS(){
	...
	$GetStyle .= "
		...
		#navigation-sticky.trans-nav {
			background: " . hex2rgb(of_get_option('menu_background_color',$prof_default) , of_get_option('menu_background_color_opacity',$prof_default)) . ";
		}
		
		.darken {
			background-color: " . hex2rgb(of_get_option('sticky_menu_background_color',$prof_default) , of_get_option('menu_background_color_opacity_sticky',$prof_default)) . " !important;
		}	
